...
class Post_Service:
    ...
    def update_post(self,request:postSchema.Post,db:Session,id,current_user_id:int):
        ...
        if get_post.author_id != current_user_id:
            raise HTTPException(
    ...
    def delete_post(self, db: Session, post_id: int, current_user_id: int):
        post_query = db.query(post.Post).filter(post.Post.id == post_id)
        ...
        if not post_to_delete:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail=f"Post with id of {post_id} not found"
            )
